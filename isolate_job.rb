class IsolateJob < ApplicationJob
  retry_on RuntimeError, wait: 0.5.seconds, attempts: 10
  ...
  def extract_archive
    ...
    #{cgroups.present? ? (submission.enable_per_process_and_thread_time_limit ? "--no-cg-timing" : "--cg-timing") : ""} \
    #{cgroups.present? ? (submission.enable_per_process_and_thread_memory_limit ? "-m " : "--cg-mem=") : "-m "}#{Config::MAX_MEMORY_LIMIT} \
  ...
  def run
    ...
    #{cgroups.present? ? (submission.enable_per_process_and_thread_time_limit ? "--no-cg-timing" : "--cg-timing") : ""} \
    #{cgroups.present? ? (submission.enable_per_process_and_thread_memory_limit ? "-m " : "--cg-mem=") : "-m "}#{submission.memory_limit} \
