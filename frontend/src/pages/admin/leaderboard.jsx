        ...
        <Layout>
            <div className="p-4 md:p-8 bg-gray-50 dark:bg-slate-900 min-h-screen">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Student performance rankings</p>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        {topThree.map((user, index) => {
                            ...
                            return (
                                <div
                                    key={user.id}
                                    className={`bg-white dark:bg-slate-800 rounded-xl border ${podiumStyles[index].border} p-5 transition-shadow hover:shadow-md`}
                                >
                                    <div className="flex items-center justify-between mb-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                {user.roll_number && (
                                                    <p className="text-xs text-slate-400 dark:text-slate-500 font-mono mt-0.5">{user.roll_number}</p>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-3 pt-3 border-t border-slate-100 dark:border-slate-700">
                                        <div>
                                            <p className="text-lg font-bold text-slate-800 dark:text-white">{user.levels_cleared}</p>
                                            <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wide">Levels</p>
                                        </div>
                                        <div>
                                            <p className="text-lg font-bold text-slate-800 dark:text-white">{user.problems_solved}</p>
                                            <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wide">Solved</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-lg font-bold text-slate-800 dark:text-white">{Math.round(user.efficiency || 0)}%</p>
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-slate-100 dark:border-slate-700">
                                        ...
                                        <th className="text-center py-2.5 px-4 text-[11px] font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wider">Solved</th>
                                            ...
                                            >
                                                <td className="py-2.5 px-4">
                                                    <div className="flex items-center gap-1.5">
                                                        ...
                                                        <span className={`text-sm ${isTop3 ? 'font-semibold text-slate-800 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>
                                                            {user.rank}
                                                        </span>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="py-2.5 px-4">
                                                    <div className="flex items-center gap-2.5">
                                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold shrink-0 ${isTop3
                                                            ? `${podiumStyles[user.rank - 1]?.avatar || 'bg-slate-200'} text-white`
                                                            : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400'
                                                            }`}>
                                                            {getInitials(user.name)}
                                                        </div>
                                                        <span className="text-sm text-slate-700 dark:text-slate-200">{user.name || 'Unknown'}</span>
                                                    </div>
                                                </td>
