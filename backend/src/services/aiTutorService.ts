...
Never directly reveal the correct answer. Help them learn!`,
    ...
    hintProvider: `You are an expert tutor providing hints for practice questions.

CRITICAL RULES:
1. NEVER reveal the answer directly
2. ALWAYS explain the specific concept being tested in this question
3. Give hints that are SPECIFIC to this exact question, not generic advice
4. Reference the actual question content in your hint
5. Guide the student's thinking process

Your hint should:
- Explain what concept/topic this question is testing
- Point out what the student should focus on in the question
- Give a clue that helps them think in the right direction
...
export const getMCQHint = async (
    ...
    const hintInstruction = {
        subtle: 'Give a subtle hint that explains what concept this question is testing. Do NOT mention any specific option.',
        moderate: 'Explain the key concept being tested and give a clue about what to look for in the options.',
        detailed: 'Explain the concept in detail with an example, then guide them on how to evaluate the options.'
    }[hintLevel];
    ...
    const request: OllamaRequest = {
        ...
        messages: [
            ...
            {
                role: 'user',
                content: `Provide a SPECIFIC hint for this question (attempt #${attemptCount}):
...
INSTRUCTION: ${hintInstruction}

Your hint must:
1. Explain what programming concept or topic this question is about
2. Give a specific clue related to THIS question content
3. NOT mention which option is correct
...
export const generatePerformanceAnalysis = async (
    ...
    // Format question data for the prompt with more detail
    const questionsContext = questions.map((q, idx) => {
        const status = q.is_correct ? '✓ CORRECT' : '✗ INCORRECT';
        const concepts = q.concepts || 'General';
        return `Q${idx + 1}: "${q.title}" - ${status}
   Topic: ${concepts}${q.time_taken ? ` | Time: ${q.time_taken}s` : ''}`;
    }).join('\n');

    const incorrectQuestions = questions.filter(q => !q.is_correct);
    const incorrectList = incorrectQuestions.map(q => q.title).join(', ') || 'None';

    const systemPrompt = `You are an expert AI tutor analyzing a student's practice session.

IMPORTANT: Your response MUST be valid JSON only. No markdown, no explanation outside JSON.

Provide feedback that is:
1. SPECIFIC to the actual questions they got wrong
2. For FAILED questions: Explain what concept they likely misunderstood
3. For PASSED sessions: Recommend related advanced topics to learn next
4. Name specific topics/concepts, not generic advice

JSON format:
{
  "analysis": "2-3 sentences analyzing their performance. Mention specific question topics they struggled with.",
  "suggestions": ["4 specific tips referencing actual topics from their incorrect answers"],
  "nextSteps": "${isPass ? 'One specific advanced topic to study next based on what they passed' : 'One specific topic to review based on their mistakes'}"
}`;

    const userPrompt = `Analyze this ${sessionType.toUpperCase()} session:

COURSE: "${courseTitle}" - ${levelTitle}
RESULT: ${isPass ? 'PASSED ✓' : 'FAILED ✗'} (Score: ${score}%)

QUESTIONS:
${questionsContext}

${!isPass ? `QUESTIONS THEY GOT WRONG: ${incorrectList}

For each incorrect question, explain what concept they likely need to review.` : `They passed! Recommend related advanced concepts to explore next.`}
