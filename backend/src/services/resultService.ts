...
export const getSessionResultsForAdmin = async (sessionId: string) => {
  // Get session info without user check
  const sessionResult = await pool.query(
    `SELECT s.id, s.session_type, s.status, s.started_at, s.completed_at,
            s.course_id, s.level_id, s.user_id,
            c.title as course_title, l.title as level_title,
            u.name as student_name, u.roll_number as student_id
